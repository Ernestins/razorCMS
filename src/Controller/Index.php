...
class Index
{
	...
	private function data_main()
	{
		// public or preview
		if (isset($_GET["preview"]) || (!$this->login && !isset($_COOKIE["token"]))) return 'data-main="base-module"';

		// logged in
		if (!isset($_GET["edit"]) || $this->logged_in < 6) return 'data-main="admin-access-module"';

		// admin editable
		return 'data-main="admin-edit-module"';
	}
...
OUTPUT;
		...
		{
			include(RAZOR_BASE_PATH."src/theme/partial/admin-access.php");
			return true;
		}

		include(RAZOR_BASE_PATH."src/theme/partial/admin-edit.php");
		return true;
	}
