...
class RazorErrorHandler {
	...
	private function log_error($error, $log_book = 'razor-error-log')
	{
		...
		// get file contents
		$log = array();
		if (is_file(RAZOR_BASE_PATH."log/{$log_book}.php"))
		{
			$log = RazorFileTools::read_file_contents(RAZOR_BASE_PATH."log/{$log_book}.php", 'array');
		}

		// set date time
		$date_time = @date('d m Y - h:i:s', time());

		$entry = "[{$date_time}] [{$error['error']}]";
		...
		if (!is_dir(RAZOR_BASE_PATH.'log')) mkdir(RAZOR_BASE_PATH.'log');
		RazorFileTools::write_file_contents(RAZOR_BASE_PATH."log/{$log_book}.php", $log_string);
	}
